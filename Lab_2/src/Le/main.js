...
class Block {
 ...
 Block.prototype.goSplitHalf = function(rows) {
 ...
 /* =============================== */
 let str = 'я нарисовала арт на заказ';
 let length = str.length;
 let lCount = new Map(); // кол-во вхождений букв
 ...
 for(let l of str) {
	...
	if(!lCount.has(l)) {
	   ...
	   lCount.set(l, lCount.get(l) + 1);
	}
 }
 //console.log(lCount); 
 
 let lFreq = lCount; // частоты букв
 for(let l of lFreq.keys()) {
	lFreq.set(l, lFreq.get(l) / length);
 }
 //lFreq.sort((a, b) => b.value - a.value);
 lFreq = new Map([...lFreq.entries()].sort((a, b) => b[1] - a[1]));
 console.log('lFreq:', lFreq);
 
 let huffman = []; // столбцы по Хаффману
 huffman[0] = {arr: []}; // 1-й столбец
 for(let l of lFreq.keys()) {
	huffman[0].arr.push(new BinNode(lFreq.get(l), l));
 }
 //console.log('huffman: ' ,huffman);
 //console.log('huffman[0]: ' ,huffman[0]);
 
 for(let i = 1; huffman[i-1].arr.length != 1; i++) {
	huffman[i] = {arr: []};
	huffman[i-1].arr.sort((a, b) => b.value - a.value);
	huffman[i].arr = huffman[i-1].arr.slice(0, -2);
	let node = new BinNode(huffman[i-1].arr[huffman[i-1].arr.length-1].value 
	   + huffman[i-1].arr[huffman[i-1].arr.length-2].value,
	   [huffman[i-1].arr[huffman[i-1].arr.length-1], huffman[i-1].arr[huffman[i-1].arr.length-2]]);
	node.value = roundNumber(node.value, 3);
	huffman[i].arr.push(node);
	huffman[i].node = node;
 }
 
 let huffman_codes = {}; // коды для всех букв
 
 let root = huffman[huffman.length-1].node; // корневой узел
 console.log(root);
 
 let treeData = [{}];
 treeData[0].name = huffman[huffman.length-1].node.value;
 treeData[0].children = [];
 ...
 //let m1, m2;
 // debugger;
 //[m1, m2] = splitMap(lCount); // [*.size == 3, *.size == 10]
 ...
 */
 ...
 function BinNode(value, parents) {
	this.value = value;
	this.parents = parents;
	this.createBranch = function(code, node) {
	   //debugger;
	   if(typeof this.parents == "string") { // если предок буква (по факту)
		  ...
		  for(let i = 0; i < 2; i++) {
			 ...
			 let name;
			 if(typeof parents[i].parents == 'string') name = '"' + parents[i].parents.toUpperCase() + '"';
			 else name = '<node>';
 
			 node[i].name = `${parents[i].value} - ${name} - ${code + i}`;
